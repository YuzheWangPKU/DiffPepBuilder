...
class PdbDataset(data.Dataset):
    ...
    def _init_metadata(self):
        ...
        if self.is_training:
            csv_path = self.data_conf.train_csv_path
        else:
            csv_path = self.data_conf.val_csv_path
    ...
    def _create_split(self, pdb_csv):
        ...
        if self.is_training:
            ...
            if self._data_conf.num_eval_samples is not None:
                selected_samples = pdb_csv.sample(self._data_conf.num_eval_samples, random_state=42)
            else:
                selected_samples = pdb_csv
            # eval_csv = pd.DataFrame(np.repeat(selected_samples.values, self._data_conf.num_repeat_per_eval_sample, axis=0))
            # eval_csv.columns = selected_samples.columns
            eval_csv = selected_samples.copy()
            self.csv = eval_csv
            self._log.info(
    ...
    def _process_csv_row(self, processed_file_path):
        ...
        processed_feats = du.parse_chain_feats(
            processed_feats,
            center_pos_noise=self._data_conf.center_pos_noise,
            center_pos_noise_std=self._data_conf.center_pos_noise_std
        )
        del processed_feats['center_pos']

        # Only take modeled residues.
        modeled_idx = processed_feats['modeled_idx']
        del processed_feats['modeled_idx']
        processed_feats = tree.map_structure(
            lambda x: x[modeled_idx], processed_feats)
        ...
        chain_feats = {
            ...
            'coordinate_bias': torch.tensor(processed_feats['coordinate_bias']).double()
        }
        ...
        ligand_mask = processed_feats['ligand_mask']  # 1 indicates peptide binder

        # Re-number residue indices.
        chain_idx = processed_feats["chain_index"]
        res_idx = processed_feats['residue_index']

        unique_chain_idx = np.unique(chain_idx)
        chain_idx_mapping = {old_idx: new_idx for new_idx, old_idx in enumerate(unique_chain_idx)}
        new_chain_idx = np.vectorize(chain_idx_mapping.get)(chain_idx)

        new_res_idx = np.zeros_like(res_idx)
        all_chain_idx = np.unique(new_chain_idx).tolist()

        start_index = 1

        for i, chain_id in enumerate(all_chain_idx):
            chain_mask = (new_chain_idx == chain_id).astype(int)
            chain_res_idx = res_idx[chain_mask == 1]

            index_shift = start_index - np.min(chain_res_idx)
            new_res_idx += (res_idx + index_shift) * chain_mask

            # 100-residue gap between chains as in RFDiffusion
            start_index = np.max(chain_res_idx) + index_shift + 100
        ...
        final_feats = {
            ...
            'coordinate_bias': chain_feats['coordinate_bias'],
            'esm_embed': processed_feats['esm_embed'],
    ...
    def __getitem__(self, idx):
        ...
        diffused_mask = chain_feats['ligand_mask']
        if np.sum(diffused_mask) < 1:
            raise ValueError(f'Must be diffused: {pdb_name}')
        ...
        # Use average ESM embeddings for peptide binder residues when the sequence is masked.
        esm_embed = chain_feats['esm_embed']
        mean_esm_embed = np.mean(esm_embed[fixed_mask.astype(bool) & chain_feats['res_mask'].astype(bool)], axis=0)
        esm_embed[diffused_mask.astype(bool)] = mean_esm_embed
        chain_feats['esm_embed'] = esm_embed
